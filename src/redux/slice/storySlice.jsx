...
export const getAllMyStories = () => async (dispatch) => {
  const token=JSON.parse(localStorage.getItem("APP-TOKEN"))
    ...
    try {
      const response = await axios.get("http://localhost:5000/api/v1/story/allStories",{
        headers:{
    ...
    export const getStoriesByCategory = (categoryArray) => async (dispatch) => {
      ...
      try {
        const response = await axios.get(`http://localhost:5000/api/v1/story/storiesByCategory/${categoryArray}`,
          {
            headers:{
              Authorization:`Bearer ${token}`
            }
      ...
      export const likeOrDislikeStory = (singleStoryId,slideId) => async (dispatch) => {
        ...
        try {
          const response = await axios.put(`http://localhost:5000/api/v1/story/likeOrDislike/${singleStoryId}`, {slideId},
      ...
      //   try {
      //     const response = await axios.patch(`http://localhost:5000/api/v1/story/bookmarkSingleSlide/${slideId}`, 
      //       {
      //       headers:{
      //         Authorization:`Bearer ${token} `
      //           }  
      ...
      export const bookmarkSingleSlide = (slideId) => async (dispatch) => {
        ...
        try {
          const response = await axios.get(`http://localhost:5000/api/v1/story/bookmarkSingleSlide/${slideId}`, 
            {
            headers:{
              Authorization:`Bearer ${token} `
                }  
